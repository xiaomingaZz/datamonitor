...
(function($){
	$.fn.btn = function(){
		...
		this.init = function(){
			...
			var btntext = 'z-btn-text';
			if (icon == 'noicon') {
				btntext = 'z-btn-text-noicon';
				icon = '';
			}
			...
			var bntStr=[
				'<table id="',id,'" class="z-btn" cellSpacing=0 cellPadding=0 border=0><tbody><tr>',
					...
					'<TD class=z-btn-center><EM unselectable="on">',
						'<INPUT style="cursor:hand;star:expression(this.onFocus=this.blur());" type="button" class="z-button ' + btntext + ' ',icon,'"  value="',$(this).attr('value'),'"></INPUT>',
					'</EM></TD>',
