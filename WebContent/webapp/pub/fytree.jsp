...
<%@page import="tdh.*"%>
<%
	String fydm = Constant.fjm;
	String fymc = Constant.fymc;
	...
	try {
		...
		String sql = "SELECT * FROM TS_FYMC WHERE FJM LIKE '"
				+ fydm.substring(0, 1) + "%'";
		...
		if ("00".equals(fydm.substring(1, 3))) {
			...
			for (TsFymc fyxx : list) {
				...
				if ("0".equals(_fydm.substring(2, 3))
					...
					if ("KE".equals(_fydm.substring(0, 2))) {
						zymap.put("text", "铁路法院");
					} else if ("KH".equals(_fydm.substring(0, 2))) {
						zymap.put("text", "海事法院");
					} else {
						zymap.put("text",
								data.get(fyxx.getFjm().substring(0, 2)));
					}
