...
<script type="text/javascript">
	...
	$(document).ready(function(){
		...
		initTree([{'contentid':'ywcbContent','treeid':'ywcbTree','inputid':'ywcb','selected':['<%=fydm%>'],'expand':true,'hide':false,
			'valueid':'ywcbValue','onClick':ywcbClick,'url':contextPath+'/webapp/sjsjc/fc/sjc_fc_tree_jx.jsp?'+param+"&FYDM="+fydm}]);
		sizeChange(true);
		loadTable(1,true);
	});
	...
	function loadTable(page,first){
		...
		loadData('./sjc_fc_data_jx.jsp?'+param,{'start':page,'limit':limit,'FYDM':fydm},function(json){
			$("#tableContent").unmask();
	...
	function doFcaj(ahdm){
		window.open(contextPath+"/webapp/ajcx/main.jsp?zlbg_ajbs="+ahdm);
	}
	...
	function doExport(){
		...
		_params += "&FYDM="+fydm;
		document.getElementById('exportFrame').src = "sjc_fc_export.jsp?"+_params;
	}
</script>
</html>